...
- §1「効果の大きさ — どう表現されるか」に v1.15 で追加した森林図の直下に、外部インタラクティブツールへの導線ボタンを設置
- リンク先：https://forest-ebm-sim-v2-n7wn-git-main-mxe050s-projects.vercel.app/
  （ブラウザ上で森林図のメタアナリシスを再計算できる ユーザー作成ツール v2）
- 緑系（#4a7c2a）の `.forest-sim-btn` カードで、本サイトの他ボタンと差別化
- 用途：「サブグループ追加・RoB が深刻な論文を削除した際の再計算」など
- キャッシュバスター `?v=7.1` → `?v=7.2`（全14箇所）

### v1.16の追加 CSS クラス
`.forest-sim-btn` `.forest-sim-mark` `.forest-sim-body` `.forest-sim-tag` `.forest-sim-title` `.forest-sim-dsc` `.forest-sim-arrow`

---

## 旧版履歴 — v1.15（2026-05-06）
...
- `3704ee7` v1.15 JAMA UG Ch23 §1 に森林図解説図（forest.png）を挿入（2026-05-06）
- v1.16 JAMA UG Ch23 §1 にフォレストプロット再解析シミュレータへの導線ボタン追加（2026-05-06）
