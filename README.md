...
- data/content-registry.json がコンテンツの正本です。現在の学習番号、旧data-idx、旧page-N、安定contentId、hrefを分けて保持します。
...
枝項目は `M3a` や `T2b` のように親項目との関係を示します。旧版の番号はURL互換の内部情報としてだけ保持し、画面には表示しません。
...
- 2026-08-27: 全56項目と9個のルートHTMLを再監査し、未登録だったY先生のEBM実践リンク集を追加。旧番号表示を新しい学習番号へ統一し、スマホ用サイドメニューを補強。
