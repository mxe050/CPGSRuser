...
本編25章、入門・案内ハブ、独立した詳細ページを、5区分の目次と14領域の学習マップで結んでいます。コンテンツ台帳は57項目です。

## 学習番号

表示番号はST（入口）、E（EBM）、M（GRADEの流れ）、S（SoF・効果・閾値）、T（詳細）、R（資料）の体系です。M3aなどは枝項目を示します。旧ページ番号との対応は内部に保持します。
